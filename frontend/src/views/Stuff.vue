  ...
  mounted() {
    ...
    DataService.getAllInfo(userSession)
      .then((response) => {
        ...
        this.stuff_heros = response.data[6];
      })
      .then(() => {
      ...
      .then(() => {
        /* check stuff avaible for this heros*/
        let heros = this.heros;
        let stuff = this.stuff_heros;
        for (let x in stuff) {
          if (stuff[x].nom == heros) {
            console.log(stuff[x].stuffs);
            this.stuffAuthorized = stuff[x].stuffs;
          }
        }
      })
      .catch((e) => {
